...
import time
title("[Sniff Some Bitches | [Vanis] Loading...")
cwd = os.getcwd()
os.system('cls') 
print(f"""{blue}
                ___   ___            
...
{gray}[{blue}!{gray}]{white} Make sure to use a vpn! So I Dont Log YOU :D
 """)
...
input()
title("[Sniff Some Bitches | [Vanis]  Loading Tokens...")

try:
    with open("tokens.txt", "r") as f:
        tokens = [x.strip() for x in f.readlines()]
    if tokens == []:
        title("[Sniff Some Bitches | [Vanis]  No Tokens Found !!")
    ...
    title("[Sniff Some Bitches | [Vanis] Loaded Tokens")
...
def checker(tokenz):
    ...
    for token in tokenz:
        ...
        try:
            ...
            pass
        title(f"[Sniff Some Bitches | [Vanis]  Checked: {len(working_tokens) + invalid + duplicate + error}/{len(tokens)} // Valid: {len(working_tokens)} - Email: {len(email)} - Phone: {len(phone)} // Invalid: {invalid} // Duplicate: {duplicate} // Rate Limit: {rate_limit} // Error: {error}")
...
title(f"[Sniff Some Bitches | [Vanis]  Sniffing // Completed: {done}")
...
def sniff(tokens):
    ...
    for token in tokens:
        ...
        title(f"[Sniff Some Bitches | [Vanis]  Sniffing // Completed: {done}/{len(working_tokens)}")
...
title("[Sniff Some Bitches | [Vanis] Completed.")
